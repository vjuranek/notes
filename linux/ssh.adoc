...
If the key is protected by passphrase, ssh may skip the key even when explicitly provided (`ssh -i`).
It can be work-around by adding the key using `ssh-add` or by adding this into `~/.ssh/config`:
